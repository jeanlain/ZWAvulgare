...
# we optimize speed by using vectorization as much as possible. All contigs are treated as the same time.
...
ni = 10L # the number of individuals per pool in our experiment
...
simulate <- function(d = 0.5) {
  ...
  nZ <- setNames(as.data.table(replicate(4, rbinom(nCont, ni, d), simplify = F)), c("WXAd", "WXAs", "ZMd", "ZMs"))

  # we need to correct these for sons
  nZ[, c("WXAs", "ZMs") := .(ni - WXAs, ni - ZMs)]
  ...
  for (pool in 1:length(F1reads)) {
    ...
    # will have number 0
    readHapl[readHapl] <- uReads[[pool]][readHapl, rbinom(.N, 1L, nZs[contig] / ni) + 1L]
  ...
  computeProb <- function(f1reads, group) {
    ...
    probas <- do.call(pFgivenCountsHapl, readCount[uContigs, -"contig"])
  }
  ...
  res <- data.table(
    ...
    n = nZ[uContigs, WXAd + ni - WXAs + ZMd + ni - ZMs], # n is the true number of recombinants
